...
L'applicazione è ispirata, a livello concettuale, a servizi dei gestione delle note come Google Keep. L'app dovrà consentire a tutti gli utenti di creare, modificare ed eliminare note, organizzandole tramite etichette (tag) e associando eventuali allegati.

\section{Setup del progetto e scelte architetturali}
\label{sec:setup-architettura}
Il progetto è stato generato tramite il CLI JHipster v8.11.0 come applicazione monolitica, scelta motivata dal fatto che consente di concentrare l’analisi sulle pratiche di testing e automazione, evitando la complessità aggiuntiva tipica di un’architettura a microservizi. Di seguito elenco le opzioni che il CLI mette a disposizione al momento della creazione:
...
L'operazione di aggiornamento della nota con il tag ``estraneo'' viene eseguita tramite endpoint REST e il comportamento atteso è il rifiuto della richiesta con codice HTTP \texttt{400}.
...
L'utente autenticato viene recuperato tramite \texttt{SecurityUtils.getCurrentUserLogin()}, utility per ottenere il login associato al contesto di sicurezza corrente. In questo modo la regola di dominio resta centralizzata nel service layer e non dipende dal client.

\subsubsection{Fase Refactor: consolidamento e pulizia}
\label{subsec:refactor}

Una volta superato il test, il codice è stato rifinito separando chiaramente la logica di validazione dal resto del flusso di aggiornamento. In particolare, il controllo è stato estratto nel metodo privato \texttt{validateOwnedTags(...)}, con due vantaggi principali.

Il vantaggio è la riduzione della duplicazione: lo stesso controllo può essere richiamato non solo in \texttt{update(...)}, ma anche in \texttt{save(...)} e in \texttt{partialUpdate(...)}.
...
\begin{itemize}
	...
	\item il service REST \texttt{note.service.spec.ts};
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=\textwidth]{cucumber.JPG}
...
Per testare l'applicazione dal punto di vista dell'utente finale viene integrato Cypress, uno strumento dedicato ai test end-to-end per applicazioni web. Cypress consente di eseguire il browser in modalità controllata e simulare interazioni reali con l'interfaccia Angular, verificando non solo la correttezza delle singole chiamate HTTP, ma soprattutto il comportamento complessivo dell'applicazione durante l'interazione utente.

I test Cypress auto generati dal progetto coprono già diversi aspetti dell’applicazione, in particolare:
...
I test generati automaticamente mostrano già la struttura generale degli scenari E2E: apertura della pagina dell’entità, accesso al form di creazione, visualizzazione della pagina di dettaglio, modifica e ritorno alla lista.
...
i test Cypress possono essere lanciati sia in ambiente di sviluppo sia in modalità automatizzata. Durante lo sviluppo conviene utilizzare l'interfaccia grafica di Cypress, che consente di osservare passo per passo i comandi eseguiti, il DOM renderizzato e le chiamate HTTP intercettate. In fase di esecuzione automatizzata, invece, la suite può essere avviata in modalità \emph{headless}, adatto alle pipeline CI.

\section{Mutation Testing con PIT}
\label{sec:pit}

Per valutare la robustezza effettiva della suite dei test è stato introdotto il mutation testing tramite PIT (Pitest). A differenza della semplice copertura del codice, il mutation testing introduce modifiche artificiali nel codice sorgente, chiamate \textit{mutanti}, per verificare se i test sono in grado di individuare tali variazioni. Se un mutante viene rilevato da almeno un test, esso viene considerato ``ucciso''; in caso contrario, indica una possibile debolezza nella suite di test.

Per utilizzare PIT con maven basta introdurre la configurazione nel pom.xml.
...
\begin{minted}{xml}
	...
	<targetTests>
	<param>com.giovannip.*Test</param>
	</targetTests>
...
\todo{mostrare i risultati del pitest}

\section{Performance Testing con Gatling}
\label{sec:gatling}

JHipster integra automaticamente Gatling come strumento per l'esecuzione di test di performance sulle API REST. Gatling consente di simulare un numero elevato di utenti concorrenti e di raccogliere metriche quali tempo medio di risposta, throughput e percentili di latenza.

\todo{mostrare e analizzare i test pregenerati}

Nel contesto della presente tesi i test di performance non costituiscono il focus principale dell'analisi. Tuttavia, la presenza di Gatling all'interno dello stack JHipster dimostra come l'ecosistema supporti anche la verifica di requisiti non funzionali, permettendo di estendere il processo di testing oltre i livelli unitari e di integrazione.
...
La pipeline è stata implementata tramite Jenkinsfile dichiarativo. La strategia adottata differenzia le attività in base al tipo di branch e al contesto di esecuzione:
...
Sul branch \texttt{dev} viene eseguita una suite più completa, includendo test di integrazione e la generazione di report. vengono raccolti report JUnit (\texttt{surefire}) e (\texttt{failsafe}), vengono pubblicati report di code coverage tramite JaCoCo, vengono pubblicati report Cucumber quando presenti.
...
Per la valutazione automatica della qualità del codice è stato integrato SonarQube all'interno della pipeline Jenkins. L'analisi statica viene eseguita sul branch \texttt{dev} e utilizza i report di copertura generati da JaCoCo.
...
La Figura~\ref{fig:sonar-quality-gate} mostra la pagina principale del progetto su SonarQube, nella quale è possibile osservare immediatamente l’esito del \textit{Quality Gate}. La Figura~\ref{fig:sonar-overview} riporta invece la sezione \textit{Overview}, che raccoglie le principali metriche di qualità del progetto. Infine, la Figura~\ref{fig:sonar-issues} mostra la lista dei problemi individuati dall’analisi statica, tra cui \textit{code smells} e potenziali bug.
...
Sul branch \texttt{main} la pipeline esegue una build di produzione e produce un’immagine Docker tramite \texttt{jib:build} per costruire l’immagine senza richiedere un Dockerfile esplicito per il backend, pubblicandola su un registry locale.
...
\subsubsection{Come fare deploy}
...
\begin{listing}[H]
\caption{Esempio semplificato di deploy del container}
