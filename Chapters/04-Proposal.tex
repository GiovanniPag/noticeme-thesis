...
L'applicazione è ispirata, a livello concettuale, a servizi di gestione delle note come Google Keep. L'app dovrà consentire a tutti gli utenti di creare, modificare ed eliminare note, organizzandole tramite etichette (tag) e associando eventuali allegati.
...
Per integrare PIT con JHipster basterà aggiungere il plugin con maven tramite la configurazione del pom.xml. come mostrato nel Listing~\ref{lst:pit-plugin}.
...
Il Listing~\ref{lst:gatling-scenario} mostra un estratto dello scenario principale della simulazione.

\captionsetup{type=listing}
\captionof{listing}{Estratto della simulazione Gatling per l'entità Note}
...
Al termine dell'esecuzione Gatling genera automaticamente un report HTML contenente le principali metriche di performance come si può vedere nella Figura~\ref{fig:gatling-report}.
...
Tra le principali estensioni possibili vi sono la simulazione di diversi profili di utenti (ad esempio utenti anonimi e autenticati, admin e normali), diverse sequenze di chiamate API che emulano diversi comportamenti sull'app, aumento progressivo del numero di utenti virtuali per valutare la scalabilità del sistema.
